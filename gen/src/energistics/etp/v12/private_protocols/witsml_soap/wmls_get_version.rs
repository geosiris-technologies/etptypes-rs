...
fn wmls_getversion_avro_schema() -> Option<Schema> {
...
impl AvroSerializable for WMLS_GetVersion {}

impl AvroDeserializable for WMLS_GetVersion {
    fn avro_deserialize<R: Read>(input: &mut R) -> AvroResult<WMLS_GetVersion> {
        let record = from_avro_datum(&wmls_getversion_avro_schema().unwrap(), input, None).unwrap();
