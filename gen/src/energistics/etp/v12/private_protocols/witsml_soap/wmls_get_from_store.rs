...
fn wmls_getfromstore_avro_schema() -> Option<Schema> {
...
impl AvroSerializable for WMLS_GetFromStore {}

impl AvroDeserializable for WMLS_GetFromStore {
    fn avro_deserialize<R: Read>(input: &mut R) -> AvroResult<WMLS_GetFromStore> {
        let record =
            from_avro_datum(&wmls_getfromstore_avro_schema().unwrap(), input, None).unwrap();
