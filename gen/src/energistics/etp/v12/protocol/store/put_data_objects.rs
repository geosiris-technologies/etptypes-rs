...
fn putdataobjects_avro_schema() -> Option<Schema> {
...
impl Schemable for PutDataObjects {
    fn avro_schema(&self) -> Option<Schema> {
        putdataobjects_avro_schema()
    }
...
impl AvroSerializable for PutDataObjects {}

impl AvroDeserializable for PutDataObjects {
    fn avro_deserialize<R: Read>(input: &mut R) -> AvroResult<PutDataObjects> {
        let record = from_avro_datum(&putdataobjects_avro_schema().unwrap(), input, None).unwrap();
        from_value::<PutDataObjects>(&record)
    }
}

impl ETPMetadata for PutDataObjects {
...
impl PutDataObjects {
    pub fn as_protocol_message(&self) -> ProtocolMessage {
        ProtocolMessage::Store_PutDataObjects(self.clone())
    }
}

impl Default for PutDataObjects {
    /* Protocol 4, MessageType : 2 */
    fn default() -> PutDataObjects {
        PutDataObjects {
