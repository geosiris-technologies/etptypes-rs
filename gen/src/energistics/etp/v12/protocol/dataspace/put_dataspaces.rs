...
fn putdataspaces_avro_schema() -> Option<Schema> {
...
impl Schemable for PutDataspaces {
    fn avro_schema(&self) -> Option<Schema> {
        putdataspaces_avro_schema()
    }
...
impl AvroSerializable for PutDataspaces {}

impl AvroDeserializable for PutDataspaces {
    fn avro_deserialize<R: Read>(input: &mut R) -> AvroResult<PutDataspaces> {
        let record = from_avro_datum(&putdataspaces_avro_schema().unwrap(), input, None).unwrap();
        from_value::<PutDataspaces>(&record)
    }
}
...
impl PutDataspaces {
    pub fn as_protocol_message(&self) -> ProtocolMessage {
        ProtocolMessage::Dataspace_PutDataspaces(self.clone())
    }
}
