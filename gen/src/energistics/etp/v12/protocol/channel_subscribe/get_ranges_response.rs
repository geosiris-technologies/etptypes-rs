...
fn getrangesresponse_avro_schema() -> Option<Schema> {
...
impl Schemable for GetRangesResponse {
    fn avro_schema(&self) -> Option<Schema> {
        getrangesresponse_avro_schema()
    }
...
impl AvroSerializable for GetRangesResponse {}

impl AvroDeserializable for GetRangesResponse {
    fn avro_deserialize<R: Read>(input: &mut R) -> AvroResult<GetRangesResponse> {
        let record =
            from_avro_datum(&getrangesresponse_avro_schema().unwrap(), input, None).unwrap();
        from_value::<GetRangesResponse>(&record)
    }
}
...
impl GetRangesResponse {
    pub fn as_protocol_message(&self) -> ProtocolMessage {
        ProtocolMessage::ChannelSubscribe_GetRangesResponse(self.clone())
    }
}
