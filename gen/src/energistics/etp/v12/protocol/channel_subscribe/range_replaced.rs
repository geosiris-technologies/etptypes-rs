...
fn rangereplaced_avro_schema() -> Option<Schema> {
...
impl AvroSerializable for RangeReplaced {}

impl AvroDeserializable for RangeReplaced {
    fn avro_deserialize<R: Read>(input: &mut R) -> AvroResult<RangeReplaced> {
        let record = from_avro_datum(&rangereplaced_avro_schema().unwrap(), input, None).unwrap();
...
impl RangeReplaced {
    pub fn as_protocol_message(&self) -> ProtocolMessage {
        ProtocolMessage::ChannelSubscribe_RangeReplaced(self.clone())
    }
}
