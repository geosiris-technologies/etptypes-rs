...
fn objectactivestatuschanged_avro_schema() -> Option<Schema> {
...
impl Schemable for ObjectActiveStatusChanged {
    fn avro_schema(&self) -> Option<Schema> {
        objectactivestatuschanged_avro_schema()
    }
...
impl AvroSerializable for ObjectActiveStatusChanged {}

impl AvroDeserializable for ObjectActiveStatusChanged {
    fn avro_deserialize<R: Read>(input: &mut R) -> AvroResult<ObjectActiveStatusChanged> {
        let record = from_avro_datum(
            &objectactivestatuschanged_avro_schema().unwrap(),
            input,
            None,
        )
        .unwrap();
