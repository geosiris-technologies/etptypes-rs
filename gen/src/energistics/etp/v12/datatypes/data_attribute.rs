...
fn dataattribute_avro_schema() -> Option<Schema> {
...
impl AvroSerializable for DataAttribute {}

impl AvroDeserializable for DataAttribute {
    fn avro_deserialize<R: Read>(input: &mut R) -> AvroResult<DataAttribute> {
        let record = from_avro_datum(&dataattribute_avro_schema().unwrap(), input, None).unwrap();
