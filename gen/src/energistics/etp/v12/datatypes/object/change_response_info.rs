...
fn changeresponseinfo_avro_schema() -> Option<Schema> {
...
impl Schemable for ChangeResponseInfo {
    fn avro_schema(&self) -> Option<Schema> {
        changeresponseinfo_avro_schema()
    }
...
impl AvroSerializable for ChangeResponseInfo {}

impl AvroDeserializable for ChangeResponseInfo {
    fn avro_deserialize<R: Read>(input: &mut R) -> AvroResult<ChangeResponseInfo> {
        let record =
            from_avro_datum(&changeresponseinfo_avro_schema().unwrap(), input, None).unwrap();
        from_value::<ChangeResponseInfo>(&record)
    }
}

impl ChangeResponseInfo {
    /* Protocol , MessageType :  */
    pub fn default_with_params(response_timestamp: i64) -> ChangeResponseInfo {
        ChangeResponseInfo {
            response_timestamp,
            changes: HashMap::new(),
        }
